...
export const AddPropertyDialog = ({ isOpen, onClose, onSubmit, property }) => {
  ...
  useEffect(() => {
    if (isOpen && property) {
      ...
      setFormData({
        ...
        biddingEndsAt: formatLocalDate(property.biddingEndsAt),
      });
      if (property.dpImage)
  ...
  useEffect(() => {
    if (showMap && mapRef.current && !mapInstanceRef.current && window.L) {
      const defaultLocation = [27.7172, 85.324];
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      Object.keys(formData).forEach((key) => {
        if (key !== "images" && key !== "dpImage")
          data.append(key, formData[key]);
      });
      if (formData.dpImage) data.append("dpImage", formData.dpImage);
      formData.images.forEach((image) => {
        if (image instanceof File) data.append("images", image);
      });
      ...
      <div className="bg-white rounded-[2rem] shadow-2xl w-full max-w-[800px] max-h-[95vh] flex flex-col overflow-hidden border border-gray-100">
        ...
        <div className="p-6 sm:p-8 space-y-6 sm:space-y-8 overflow-y-auto scrollbar-hide">
          ...
          <div className="space-y-2">
            ...
            <label
              htmlFor="dpImage"
              className="flex items-center justify-center w-full h-48 sm:h-64 border-4 border-dashed border-gray-100 rounded-[2rem] cursor-pointer hover:border-[#B59353] hover:bg-gray-50 transition-all overflow-hidden bg-gray-50"
            >
              {dpImagePreview ? (
                <img
                  src={dpImagePreview}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
            <div className="space-y-2">
              <label className="text-xs sm:text-sm font-black text-gray-700 uppercase tracking-widest">
                Property Type
              </label>
              <select
                name="propertyType"
                ...
                className="w-full px-5 py-4 border-2 border-gray-100 rounded-2xl focus:border-[#B59353] outline-none font-black text-gray-700 bg-white appearance-none"
              >
          ...
          <div className="p-4 sm:p-5 bg-gray-50 rounded-2xl border-2 border-gray-100 space-y-4">
            <div className="flex items-center justify-between">
              <div>
                <label className="text-sm sm:text-base font-black text-gray-800">
                  Enable Bidding
                </label>
                <p className="text-xs font-bold text-gray-400">
                  Allow users to place offers
                </p>
              </div>
              <button
                type="button"
                onClick={() =>
                  setFormData((prev) => ({
                    ...prev,
                    isBidding: !prev.isBidding,
                  }))
                }
                className={`relative inline-flex h-7 w-14 items-center rounded-full transition-colors ${formData.isBidding ? "bg-red-500" : "bg-gray-300"}`}
              >
                <span
                  className={`inline-block h-5 w-5 transform rounded-full bg-white transition-transform ${formData.isBidding ? "translate-x-8" : "translate-x-1"}`}
                />
              </button>
            </div>
            ...
            {formData.isBidding && (
              <div className="pt-4 border-t border-gray-200 animate-in fade-in slide-in-from-top-2">
                <label className="text-xs font-black text-gray-700 uppercase tracking-widest mb-2 block">
                  Bidding Ends At
                </label>
                <input
                  type="datetime-local"
                  ...
                  className="w-full px-5 py-3 border-2 border-gray-200 rounded-xl focus:border-[#B59353] outline-none font-bold text-gray-700 bg-white"
                />
              </div>
            )}
          </div>
          ...
          <div className="space-y-2">
            ...
            <input
              ...
              onChange={handleInputChange}
              className="w-full px-6 py-4 border-2 border-gray-100 rounded-2xl focus:border-[#B59353] outline-none font-black text-xl sm:text-2xl"
              placeholder="0.00"
            />
            {formData.isBidding && (
              <p className="text-xs font-bold text-blue-600 pl-1">
                Starting bid price
              </p>
            )}
          </div>
          ...
          <div className="space-y-2">
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              <input
                ...
                onChange={handleInputChange}
                className="flex-1 px-5 py-4 border-2 border-gray-100 rounded-2xl focus:border-[#B59353] outline-none font-bold"
                placeholder="Address..."
              />
              <button
                type="button"
                onClick={() => setShowMap(!showMap)}
                className="px-8 py-4 bg-[#B59353] text-white rounded-2xl font-black flex items-center justify-center gap-2 hover:bg-[#a68546] transition-colors"
              >
                <MapPin size={20} /> {showMap ? "Hide" : "Map"}
              </button>
            </div>
            {showMap && (
              <div
                ref={mapRef}
                className="w-full h-64 sm:h-80 rounded-[2rem] border-2 border-gray-100 mt-4 overflow-hidden shadow-inner"
              />
            )}
          </div>
          ...
          <div className="grid grid-cols-4 gap-3 sm:gap-4">
            {["living", "beds", "kitchen", "washroom"].map((room) => (
              <div key={room} className="space-y-1">
                <label className="block text-[10px] sm:text-xs font-black text-gray-400 uppercase text-center tracking-wide">
                  {room}
                </label>
                <input
                  type="number"
                  ...
                  onChange={handleInputChange}
                  min="0"
                  className="w-full px-2 py-3 sm:py-4 border-2 border-gray-100 rounded-2xl font-black text-center text-lg outline-none focus:border-[#B59353]"
                />
              </div>
            ))}
          </div>
          ...
          <div className="space-y-2">
            ...
            <textarea
              ...
              className="w-full px-5 py-4 border-2 border-gray-100 rounded-2xl focus:border-[#B59353] outline-none font-bold resize-none text-sm sm:text-base"
            />
          </div>
          ...
          <div className="space-y-4">
            ...
            <label
              htmlFor="gallery"
              className="inline-flex items-center gap-2 px-6 py-3 border-2 border-dashed border-gray-200 rounded-2xl cursor-pointer hover:bg-gray-50 font-black text-gray-400 transition-all text-sm sm:text-base"
            >
            ...
            <div className="grid grid-cols-3 sm:grid-cols-4 gap-3 sm:gap-4 mt-2">
              {imagePreviews.map((preview, index) => (
                <div key={index} className="relative group aspect-square">
                  <img
                    src={preview}
                    className="w-full h-full object-cover rounded-2xl border-2 border-gray-50 shadow-sm"
                  />
                  <button
                    type="button"
                    onClick={() => removeGalleryImage(index)}
                    className="absolute -top-2 -right-2 bg-red-600 text-white rounded-full p-1.5 shadow-xl hover:scale-110 transition-all z-10 opacity-100 sm:opacity-0 sm:group-hover:opacity-100"
                  >
